...
Download the data here: https://www.bundeswahlleiterin.de/bundestagswahlen/2017.html. **The workflow is exactly the same as for the 2021 data!** So you can just adapt the code from above.

YOUR TURN!

You can add your code to the code chunks below. If you struggle you can have a look at the end of this document. There, you can find helpful code.
...
Next, we need to transform the data again: 1) we want to get rid of all rows regarding the federal and regional level, and keep the constituency level, 2) we are only interested in the "Zweitstimme", 3) then, select only  party name, vote share, and constituency_number, 4) we are only interested in the vote share of the AfD.

```{r transform-election-data-2017, message=FALSE, warning=FALSE}

```

Here, we load the 2017 structural data. Due to the encoding of the csv file this is a little more complicated. So you can just run the code below. However, you have to adapt the file path!
...
btw17_structural <- read_delim("C:/Users/David/Downloads/btw2017_strukturdaten.csv", # you have to adapt the file path
  ...
  mutate(constituency_number = sprintf("%03d", as.numeric(constituency_number))) # e.g., this changes the constituency number from 1 to 001, 2 to 0002, etc. We need to to this transformation that we can merge the data frame with the 2017 election data
```

Now, merge both data frames by constituency number, rename "Arbeitslosenquote März 2017 - insgesamt" to "unemployment_rate", and create the "east" variable as above.

```{r merge-election-data-with-structural-data-2017, message=FALSE, warning=FALSE}

```

Now we can plot our 2017 data! Try to create a scatterplot of unemployment rate and AfD vote share.
...
Here, we combine the data from 2017 and 2021. 1) We start by transforming the data frames separately: We want to focus on the following subset of variables: constituency_number, vote_share, unemployment_rate, east, 2) In addition, we create for each data frame a year variable using "tibble::add_column(year = XXXX)", 3) we can use the function "bind_rows()" to combine both data frames.
...
Here we want to plot our new data frame faceted by year:
...
Now, we create our custom theme. Use the theme from session05_example.Rmd!
...
```{r hints-load-election-data_2017, message=FALSE, warning=FALSE}
# You can find the data to download also here: https://www.bundeswahlleiterin.de/bundestagswahlen/2021/ergebnisse.html
...
#### Transform 2017 election data

```{r hints-transform-election-data-2017, message=FALSE, warning=FALSE}
...
#### Merge 2017 data frames

```{r hints-merge-election-data-with-structural-data-2017, message=FALSE, warning=FALSE}
...
```{r hints-relationship-AfD-vote-share-unemployment-rate-2017}
ggplot(data = btw17,
       mapping = aes(x = unemployment_rate,
                     y = vote_share)) +
  geom_point()
...
```{r hints-plot-relationship-AfD-vote-share-unemployment-rate-east-west-2017}
ggplot(data = btw17,
       mapping = aes(x = unemployment_rate,
                     y = vote_share,
                     color = east)) +
...
#### Combine 2017 and 2021 data
```{r hints-combine 2017 and 2021 data}
btw2021 <- btw21 %>% 
...
#### Create and store fancy theme
```{r hints-create-and-store-fancy-theme}
